...
# 参数初始化
OUTPUT_BIN_EXE_FILE=${1:-"{{MODULE_NAME}}"} # 第一个位置参数为待输出的二进制文件名字参数
COMMIT=${2:-$(git rev-parse --short HEAD)} # 第二个位置参数为当前构建的commitid
TARGETARCH=${3:-"amd64"} # 第三个位置参数为当前要构建的目标架构
...
BUILD_CMD="go mod tidy && rm -f ${OUTPUT_BIN_EXE_FILE} && CGO_ENABLED=0 GOOS=linux GOARCH=$TARGETARCH go build -ldflags '-extldflags \"-static\"' -o ${OUTPUT_BIN_EXE_FILE}"
