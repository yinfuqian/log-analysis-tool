/**
 * jira.mjs —— jira-code 技能使用的 Jira 客户端（Jira Server / Data Center，REST API v2）
 *
 * 本文件由 jira-gate-1 技能的同名脚本派生（vendored），读取单子、下载附件与读写评论的行为与其一致。
 * 文件内保留的「流转（transition）」与「门禁字段（gate field）」相关函数 jira-code 不使用：
 * 本技能只读单子、只写评论，不改状态、不写自定义字段，禁止调用这些函数。
 *
 * 运行环境约束（node_repl 沙箱）：
 ...
 *
 * 典型用法（shell）：
 *   node <SKILL_DIR>/scripts/jira-cli.mjs get <KEY|URL>
 */
